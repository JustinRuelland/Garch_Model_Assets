...
echantillon_eta_square_1000_2 <-  matrix(data = (rt(n= taille_relative_echantillon*Nb_echantillon, df = 4 )*sqrt((4-2)/4))**2, nrow = Nb_echantillon)

echant_eta_square_1000 <- as.list(as.data.frame(t(echantillon_eta_square_1000)))

echant_eta_square_1000 <- as.list(as.data.frame(t(echantillon_eta_square_1000_2)))
...
serie <- serie_rendement(eta = echant_eta_square_1000[[2]], sigma_init = sigma_sq_0[2], omega = omega_0, alpha = alpha_0, beta = beta_0)
...
ci <- c(0.00001, -1, 0, 0, -1, -0.999)

test1 <- serie

optimisation <- constrOptim(theta = c(0.0005,0.8,0.1), f = quasi_max, grad = NULL, ui = ui, ci = ci, serie = test1)
...
for(i in 1:taille_z){
  ...
  optimisation <- constrOptim(theta = c(0.1,0.4,0.4), f = quasi_max, grad = NULL, ui = ui, ci = ci, serie = serie_test)
...
matrice_test <- as.data.frame(rbind(matrix(matrice[,"omega"], nrow = taille_z), matrix(matrice[,"alpha"], nrow = taille_z), matrix(matrice[,"beta"], nrow = taille_z)))
col <- as.data.frame(matrix(data = c(rep(x = "Omega", taille_z), rep(x = "Alpha", taille_z), rep(x = "Beta", taille_z)), nrow=3*taille_z))

matrice_test <- cbind(matrice_test, col)
colnames(matrice_test) <- c("Val","Parametre")
