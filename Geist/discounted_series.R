...
data_disc = transform_csv_with_discount(data,3/365)

rendement = data_disc$rendement
rendement = rendement[!is.na(rendement)]
...
p_val_disc = func_backtest(rendement,-1.96,1.96,8*n%/%10,FALSE)$p.value

rendements_no_disc = c(data_no_discount$rendement)
p_val_no_disc = func_backtest(rendements_no_disc,-1.96,1.96,8*n%/%10,FALSE)$p.value
